...
ちなみに、設計当初は`Handlers`を組み合わせていろんな性質を持つModuleをつくれるようにしたい(_Javaの[lombok](http://dev.classmethod.jp/server-side/java/use_lombok_short/)アノテーションみたいな感じで簡単に付与できるようにしたい_)と思っていたのですが、handlerの上書きが起きるのでやめました…。
