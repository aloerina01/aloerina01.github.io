---
layout: post
title: Babelの設定を見直すための逆引き利用ガイド
outline: Babelの使い方って忘れません？ polyfillの使い分け理解していますか？ pluginの使い方覚えていますか？ 私は忘れていました…。そんなわけで「ES6とかで書いたものをコンパイルしてくれる」くらいの認識だった私が自在に設定できるようになるまでに覚えたことを、用途別にまとめました。一緒に.babelrcを見直してみませんか？
...
「最新の記法でJSを書く」＝「この2つを含むソースをBabeる(Babelでコンパイルする)」 ということになりますね。それには以下の2つのことが必要です。
...
// bundle後 使っているものだけimportするように変換される
...
##### グローバルを汚染せずにpolyfillを使いたい、複数箇所ファイルでpolyfillを使いたい
...
|                        |                      メリット                     |                  デメリット                  |
|:————————————:|:————————————————————————:|:——————————————————————:|
| require babel-polyfill |   全polyfillを利用できる  必要な分だけ入れられる   |        二重読み込み制約のリスクがある  グローバルが汚染される        |
|      webpackで結合     |   全polyfillを利用できる  二重読み込みのリスクがない   |       useBuiltIns オプションが効かない  グローバルが汚染される       |
|    runtime transform   |               グローバルを汚染しない  必要な分だけ変換される              | インスタンスメソッドのpolyfillは利用できない |
...
また、上述した`useBuiltIns`のような便利なオプションを使うことができる利点があります。個人的にありがたいと思っているのが**[Env preset · Babel](https://babeljs.io/docs/plugins/preset-env/#targets)**オプションです。ブラウザやNodeのバージョンを指定できるオプションで、指定されたバージョンに必要なpolyfillのみを入れてくれるようになります。

参考： [Env preset # Options](https://babeljs.io/docs/plugins/preset-env/#options)


#### ES2015+のソースをそのままminifyできるってよ
これはBabelの公式ブログ読んでて見つけて得た程度の知識なのですが、「古いブラウザで動かす必要がないならそのまま圧縮すれば？」ということで**[Babili (babel-minify) · Babel](https://babeljs.io/blog/2016/08/30/babili)**というものがあるそうです。
...
uglify-esでサポートされていない構文であっても圧縮可能なので、その点は利がありそうです。ただ、執筆時点(2018/03/16)ではBeta版ということもあって、個人的にはまだ使うことはなさそうです。


### おわりに

公式ドキュメントを読み返してみたらけっこう知らないことがありました。それでも何となく雰囲気で使えていたので大きく困ることはなかったのですが、頭が整理されて今後新しいプロジェクトを立ち上げるときに迷わなくて済みそうなので整理した甲斐がありそうです。

それと公式ドキュメントがけっこう説明不足感が強くて、しかも日本語の資料もそんなに多く見当たらなくて、整理に時間かかり苦労したので、同じ苦労を誰かがせずに済むといいなと思います。  間違っているところを見つけた方は[@aloerina_](https://twitter.com/aloerina_)までご一報ください。
