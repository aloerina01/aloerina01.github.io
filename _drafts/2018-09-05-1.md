...
後述しますが、Payloadに応じてStoreの中身をどう更新するか(または更新しないか)は、Callback側で判断することになります。
...
const callback2 = (payload) => {
  dispatcher.waitFor([callbackId1]);  // どのcallbackの実行を待つかを指定します
  // do something
}
...
Dispatcherから呼び出されるCallbackは「Storeの状態を更新するもの」ですが、具体的にどのような実装がされるのでしょうか？前提としてStoreはアプリケーションのState(状態)を保持するもので、この**Stateを生成するのがCallbackの役割です**。
...
とてもシンプルですね。このActionと呼ばれるオブジェクトがDispatcherに渡されることがFluxのデータフローの起点になります。ちなみにActionオブジェクト内に定義される`type`を一般的に**ActionType**と呼びます。

また、Actionには`type`プロパティ以外に、ユースケースを実行するために必要な値を持たせることもできます。
...
FSAはReduxを使う以上守るべきものですが(守らないと動かないケースもあり得ますが)、Fluxの設計手法の中に明記されているものではありません。個人的には、Actionをどう定義すべきか迷ったときに参考にするパターンのひとつ、くらいに捉えています。
