...
* setterの代わりにStoreを更新するためのCallbackを持ち、CallbackをDispatcherに登録する
* Storeが変更されたら、イベントを発火し変更をViewに伝える

これらの性質により、StoreとViewは完全に切り離され、アプリケーションの状態を変更するレイヤーでは「どのように状態を変えるか」にのみ専念すれば良くなります。変更後の状態に応じてViewをどのように変えるかは、View側が気にかければ良いというわけです。

<div style="border-radius:10px;background-color:#f4f6f7;padding:12px 20px;margin:42px 0 48px;">

ただし実際には、Storeが完全にViewのことを意識しないで実装しきれるケースは少なく、Viewの状態を保持しなければならない場面も出てくると思います。つまりSoteには「アプリケーションの状態を保持するStore」と「Viewの状態を保持するStore」という2種類が出てくるということになります。  
とは言えこれは<a target="_blank" href="http://facebook.github.io/flux/">Flux公式</a>の外のお話。また別の機会に考えをまとめてみようと思います。

</div>

このルールのため、Storeの更新のされ方が統一される。そして変更されたらすぐにViewに伝わり、Viewへ反映される。

StateとLogicを持つ
実はロジックもここなのよ。ここで言うロジックはDispatcherに登録するcallbackのこと
Domainの状態を管理する「ロジック」
...
FluxはWebアプリケーションの設計方法の選択肢としてとても有用なものだということがわかりました。でもFluxは完璧ではありません。Fluxの問題点や苦手分野については様々なところで議論されている通りで、アプリケーションが複雑さが増せばできない場面も多いと思います。非同期処理、複雑なドメイン層の表現、アニメーション…。課題はいろいろあると思います。が、それらに立ち向かうためにも、まずは基礎を100％理解していることが必要だと思います。
