...
ここまでで「mixinの利用は必要最低限に抑える方針」が良さそうだと分かりますが、「必要最低限」とはどの程度なのでしょうか。言い換えると、**VueComponentにおいて共通部品の再利用がどの程度必要か**ということですね。
...
ViewModelはViewと1対1で結びつくので、対応するViewのための表示用のデータの成形・ローカルStateの管理・Modelの監視(VuexのStoreとのつなぎ込み等)などのロジックを持ちます。これらは全てViewのためのロジックであるため、「似ているけど別々のView」には「別々のViewModel」が結びつき、別々のViewModelは互いに影響し合わないほうが良さそうです。つまり、ViewModelは基本的には(たとえ似たような役割であっても)コードを共通化せず、独立して持つほうが良さそうと言えます。

とはいえ、完全に同じ役目をする関数が必要な場面もありますよね。

* 
* 
* 

などです。こういったものは共通化する必要がありそうです。

まとめると、ViewModel同士は関係性が薄いので、互いに影響しあう要素は極力減らしたほうがいい。ただし、明示的に同一視したいロジックに関しては共通化したほうが良い。
