...
outline: 
---
dartをつかってwebアプリのフロントエンドを作る上で必用そうなものをリストアップ
	* dartの実行環境 	
	- dartを開発するためのディレクトリ → **stagehand**
	- ローカルサーバとデバッグ環境 → **webdev**
		- 差分のwatch
		- ホットリロード
	- 開発用と本番用の環境別ビルド → **build.yaml**
		- dartをjsにする → **DDC, dart2js**
		- sassをcssにする → **sass_builder**
	- 依存パッケージ監理 → **pub, pubspec.yaml**
	- ライブラリ → …
...
### Dartのバージョン
現時点では安定版の1.24.3とstable版の2.0.0-dev.69.0がある。
